(function() {
    ...
    angular.module('application', [
        ...
        .directive("getProp", function($http, $filter) {
            // 当api link返回的数据中有数组时，且数组由数条api link组成
            // 用getProp解析这数条api link
            return {
                // template模版 当前端调用自定义(directive)标签(get-prop)时
            ...
            if ($scope.id != '') {
                $http.get(urlApi, queryParams)
                    .success(function(data) {
                        ...
                        var googleUrl = 'https://www.googleapis.com/customsearch/v1?cx=001000040755652345309%3Aosltt3fexvk&q=' 
                                        + encodeURIComponent(name) + 
                                        '&imgSize=large&num=1&fileType=jpg&key=AIzaSyBDvUGYCJfOyTNoJzk-5P9vE-dllx-Wne4',
